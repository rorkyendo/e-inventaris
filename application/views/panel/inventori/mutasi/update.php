    ...
    <div class="col-md-12">
      ...
      <div class="panel panel-inverse" data-sortable-id="form-stuff-1">
        ...
        <div class="panel-body">
          ...
          <form class="form-horizontal" method="post" action="<?php echo base_url(changeLink('panel/inventori/updateMutasi/doUpdate/'.$key->id_faktur)); ?>" enctype="multipart/form-data">
          <div class="col-md-12">
          <center>
            ...
            <div class="col-md-12">
              <div class="form-group">
                ...
                <div class="col-md-10">
                  <input type="text" class="form-control" placeholder="Masukkan Kode Faktur" name="kode_faktur" value="<?php echo $key->kode_faktur;?>" required/>
                </div>
              </div>
              <div class="form-group">
                <label class="col-md-2 control-label">Catatan Faktur</label>
                <div class="col-md-10">
                  <input type="text" class="form-control" placeholder="Masukkan Catatan Faktur" name="catatan_faktur" value="<?php echo $key->catatan_faktur;?>" required />
                </div>
              </div>
              <hr />
              <div class="panel panel-inverse">
                ...
                <div class="panel-body" id="groupInvent">
                  <?php foreach($detailFaktur as $row):?>
                    <div class="col-md-12" id="inventori<?php echo $row->id_inventori;?>">
                      <div class="col-md-12">
                        <button class="btn btn-xs btn-danger" type="button" onclick="hapus('<?php echo $row->id_inventori;?>')"><i class="fa fa-times"></i> Hapus</button>
                      </div>
                      <div class="col-md-3">
                          <label>Nama Unit</label>
                          <input type="text" value="<?php echo $row->nama_unit_awal;?>" class="form-control" readonly>
                          <input type="hidden" name="unit_awal[]" value="<?php echo $row->unit_awal;?>">
                        </div>
                        <div class="col-md-3">
                          <label>Nama Sub Unit</label>
                          <input type="text" value="<?php echo $row->nama_sub_unit_awal;?>" class="form-control" readonly>
                          <input type="hidden" name="sub_unit_awal[]" value="<?php echo $row->sub_unit_awal;?>">
                        </div>
                        <div class="col-md-3">
                          <label>Kode Inventori</label>
                          <input type="text" value="<?php echo $row->kode_inventori;?>" class="form-control" readonly>
                        </div>
                        <div class="col-md-3">
                          <label>Nama Inventori</label>
                            <input type="hidden" name="id_inventori[]" value="<?php echo $row->id_inventori;?>">
                            <input type="text" value="<?php echo $row->nama_inventori;?>" class="form-control" readonly>
                          </div>
                        <div class="col-md-6 unit">
                        <label>Pindah Ke Unit</label>
                          <select class="form-control select2" id="unit_pindah<?php echo $row->id_inventori;?>" name="unit_pindah[]" onchange="cariSubUnit('<?php echo $row->id_inventori;?>',this.value)" required>
                        ...
                        <script>
                          $('#unit_pindah<?php echo $row->id_inventori;?>').val('<?php echo $row->unit_pindah;?>')
                          $(document).ready(function(){
                            cariSubUnit('<?php echo $row->id_inventori;?>','<?php echo $row->unit_pindah;?>','<?php echo $row->sub_unit_pindah;?>')
                          });
                        </script>
                        <div class="col-md-6 sub_unit">
                        <label>Pindah Ke Sub Unit</label>
                          <select class="form-control select2" name="sub_unit_pindah[]" id="sub_unit_pindah<?php echo $row->id_inventori;?>" required>
              ...
              <div class="col-md-12">
                ...
                <a href="<?php echo base_url(changeLink('panel/inventori/daftarMutasi/')); ?>" class="btn btn-sm btn-danger pull-right">Batal</a>
              </div>
            </div>
        </div>
        </form>
...
<script>
  ...
  function cariSubUnit(id_inventori,val,sub_unit_pindah=''){
    ...
    $.ajax({
      ...
      data:{
        ...
        if (resp!='false') {
          ...
          if (sub_unit_pindah!='') {
            $("#sub_unit_pindah"+id_inventori).val(sub_unit_pindah);
          }
        }else{
          Swal.fire({
            type: 'error',
