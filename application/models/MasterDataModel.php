...
class MasterDataModel extends CI_Model {
  ...
  public function getSubSubKelompok($kd_skel,$kd_gol,$kd_bid,$kd_kel)
  {
    ...
    $this->datatables->add_column(
      'action',
       anchor(changeLink('panel/masterData/updateSubSubKelompok/$1'), '<i class="fa fa-edit"></i>', array('class' => 'btn btn-warning btn-xs')) . ' '
      . anchor(changeLink('panel/masterData/deleteSubSubKelompok/$1'), '<i class="fa fa-times"></i>', array('class' => 'btn btn-danger btn-xs', "onclick" => "return confirm('Apakah kamu yakin akan menghapus sub-sub kelompok?')")),
      'id_ssub_kel'
    );
    if (!empty($kd_gol)) {
