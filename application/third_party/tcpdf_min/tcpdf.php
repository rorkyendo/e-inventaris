...
class TCPDF {
	...
	public function Output($name='doc.pdf', $dest='I') {
		...
		if ($this->sign) {
			...
			if (empty($this->signature_data['extracerts'])) {
				openssl_pkcs7_sign($tempdoc, $tempsign, $this->signature_data['signcert'], array($this->signature_data['privkey'], $this->signature_data['password_pengguna']), array(), PKCS7_BINARY | PKCS7_DETACHED);
			} else {
				openssl_pkcs7_sign($tempdoc, $tempsign, $this->signature_data['signcert'], array($this->signature_data['privkey'], $this->signature_data['password_pengguna']), array(), PKCS7_BINARY | PKCS7_DETACHED, $this->signature_data['extracerts']);
			}
			// read signature
	...
	protected function _Uvalue() {
		if ($this->encryptdata['mode'] == 0) { // RC4-40
			...
			return hash('sha256', $this->encryptdata['user_password_pengguna'].$this->encryptdata['UVS'], true).$this->encryptdata['UVS'].$this->encryptdata['UKS'];
		}
	}
	...
	protected function _UEvalue() {
		$hashkey = hash('sha256', $this->encryptdata['user_password_pengguna'].$this->encryptdata['UKS'], true);
	...
	protected function _Ovalue() {
		if ($this->encryptdata['mode'] < 3) { // RC4-40, RC4-128, AES-128
			$tmp = TCPDF_STATIC::_md5_16($this->encryptdata['owner_password_pengguna']);
			...
			$enc = TCPDF_STATIC::_RC4($owner_key, $this->encryptdata['user_password_pengguna'], $this->last_enc_key, $this->last_enc_key_c);
			if ($this->encryptdata['mode'] > 0) {
			...
			return hash('sha256', $this->encryptdata['owner_password_pengguna'].$this->encryptdata['OVS'].$this->encryptdata['U'], true).$this->encryptdata['OVS'].$this->encryptdata['OKS'];
		}
	}
	...
	protected function _OEvalue() {
		$hashkey = hash('sha256', $this->encryptdata['owner_password_pengguna'].$this->encryptdata['OKS'].$this->encryptdata['U'], true);
		return TCPDF_STATIC::_AESnopad($hashkey, $this->encryptdata['key']);
	}

	/**
	 * Convert password_pengguna for AES-256 encryption mode
	 * @param $password_pengguna (string) password_pengguna
	 * @return string password_pengguna
	 * @protected
	 * @since 5.9.006 (2010-10-19)
	 * @author Nicola Asuni
	 */
	protected function _fixAES256Password($password_pengguna) {
		$psw = ''; // password_pengguna to be returned
		$psw_array = TCPDF_FONTS::utf8Bidi(TCPDF_FONTS::UTF8StringToArray($password_pengguna, $this->isunicode, $this->CurrentFont), $password_pengguna, $this->rtl, $this->isunicode, $this->CurrentFont);
	...
	protected function _generateencryptionkey() {
		...
		if (!$this->encryptdata['pubkey']) { // standard mode
			if ($this->encryptdata['mode'] == 3) { // AES-256
				...
				// truncate password_penggunas
				$this->encryptdata['user_password_pengguna'] = $this->_fixAES256Password($this->encryptdata['user_password_pengguna']);
				$this->encryptdata['owner_password_pengguna'] = $this->_fixAES256Password($this->encryptdata['owner_password_pengguna']);
				// Compute U value
				...
				// Pad password_penggunas
				$this->encryptdata['user_password_pengguna'] = substr($this->encryptdata['user_password_pengguna'].TCPDF_STATIC::$enc_padding, 0, 32);
				$this->encryptdata['owner_password_pengguna'] = substr($this->encryptdata['owner_password_pengguna'].TCPDF_STATIC::$enc_padding, 0, 32);
				// Compute O value
				$this->encryptdata['O'] = $this->_Ovalue();
				// get default permissions (reverse byte order)
				$permissions = TCPDF_STATIC::getEncPermissionsString($this->encryptdata['protection']);
				// Compute encryption key
				$tmp = TCPDF_STATIC::_md5_16($this->encryptdata['user_password_pengguna'].$this->encryptdata['O'].$permissions.$this->encryptdata['fileid']);
				if ($this->encryptdata['mode'] > 0) {
					for ($i = 0; $i < 50; ++$i) {
	 ...
	 * If you don't set any password_pengguna, the document will open as usual. If you set a user password_pengguna, the PDF viewer will ask for it before displaying the document. The master password_pengguna, if different from the user one, can be used to get full access.
	 ...
	 * @param $user_pass (String) user password_pengguna. Empty by default.
	 * @param $owner_pass (String) owner password_pengguna. If not specified, a random value is used.
	...
	public function SetProtection($permissions=array('print', 'modify', 'copy', 'annot-forms', 'fill-forms', 'extract', 'assemble', 'print-high'), $user_pass='', $owner_pass=null, $mode=0, $pubkeys=null) {
		...
		if (($pubkeys !== null) AND (is_array($pubkeys))) {
			...
			// standard mode (password_pengguna mode)
		...
		$this->encryptdata['user_password_pengguna'] = $user_pass;
		$this->encryptdata['owner_password_pengguna'] = $owner_pass;
	 ...
	 * @param $private_key_password_pengguna (string) password_pengguna
	 ...
	 * @since 4.6.005 (2009-04-24)
	 */
	public function setSignature($signing_cert='', $private_key='', $private_key_password_pengguna='', $extracerts='', $cert_type=2, $info=array(), $approval='') {
		// to create self-signed signature: openssl req -x509 -nodes -days 365000 -newkey rsa:1024 -keyout tcpdf.crt -out tcpdf.crt
		// to export crt to p12: openssl pkcs12 -export -in tcpdf.crt -out tcpdf.p12
		...
		if (strlen($signing_cert) == 0) {
			$this->Error('Please provide a certificate file and password_pengguna!');
		}
		...
		$this->signature_data['password_pengguna'] = $private_key_password_pengguna;
	 ...
	 * @param $tsa_username_pengguna (string) Specifies the username_pengguna for TSA authorization (optional) OR specifies the TSA authorization PEM file (see: example_66.php, optional)
	 * @param $tsa_password_pengguna (string) Specifies the password_pengguna for TSA authorization (optional)
	 ...
	 */
	public function setTimeStamp($tsa_host='', $tsa_username_pengguna='', $tsa_password_pengguna='', $tsa_cert='') {
		$this->tsa_data = array();
		...
		if (is_file($tsa_username_pengguna)) {
			$this->tsa_data['tsa_auth'] = $tsa_username_pengguna;
		} else {
			$this->tsa_data['tsa_username_pengguna'] = $tsa_username_pengguna;
		}
		$this->tsa_data['tsa_password_pengguna'] = $tsa_password_pengguna;
		...
		switch($tag['value']) {
			...
			case 'input': {
				...
				switch ($tag['attribute']['type']) {
					...
					case 'password_pengguna': {
						if (isset($value)) {
							$opt['v'] = $value;
						}
						$prop['password_pengguna'] = 'true';
						$this->TextField($name, $w, $h, $prop, $opt, '', '', false);
						break;
					}
					case 'checkbox': {
